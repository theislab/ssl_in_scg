...
def eval_rec(
    ...
    if supervised_subset is None:
        rec_report_path = os.path.join(RESULT_PATH, "reconstruction", "rec_report.csv")
    else:
...
def test_random_model(
    ...
    # Create a new DataFrame to store the flattened data
    rec_report_path = os.path.join(
        RESULT_PATH, "reconstruction", "rec_report_comp_ssl.csv"
    )
    if not os.path.exists(rec_report_path):
        rec_report = pd.DataFrame(
            columns=["Explained Variance Uniform", "Explained Variance Weighted", "MSE"]
...
def test_models_on_ood(
    ...
    # Create a new DataFrame to store the flattened data
    rec_report_path = os.path.join(
        RESULT_PATH, "reconstruction", "rec_report_comp_" + ood_set + ".csv"
    )
    if not os.path.exists(rec_report_path):
        rec_report = pd.DataFrame(
            columns=["Explained Variance Uniform", "Explained Variance Weighted", "MSE"]
    ...
    for model_dir in model_dirs:
        ...
        # Load OOD data
        x_file_path = os.path.join(DATA_PATH, ood_set, "processed_tensor_x.hdf5")
        y_file_path = os.path.join(DATA_PATH, ood_set, "processed_tensor_y.hdf5")
...
def pca(
    RESULT_PATH: str = "/lustre/groups/ml01/workspace/till.richter/ssl_results/",
    DATA_DIR: str = "/lustre/groups/ml01/workspace/till.richter/merlin_cxg_2023_05_15_sf-log1p",
) -> None:
    """
    ...
    pca_dir = os.path.join(
        RESULT_PATH, "reconstruction", "reconstructed_data", "test_pca.npy"
    )
    ...
    if not os.path.exists(
        os.path.join(root, "self_supervision", "data", "hvg_indices.pickle")
    ):
        raise FileNotFoundError("hvg_indices.pickle does not exist")
    hvg_indices = pickle.load(
        open(os.path.join(root, "self_supervision", "data", "hvg_indices.pickle"), "rb")
    )
