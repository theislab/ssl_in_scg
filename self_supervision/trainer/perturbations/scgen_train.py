...
# Removed unused import statement for anndata
from self_supervision.data.checkpoint_utils import (
    load_last_checkpoint,
    checkpoint_exists,
)
from self_supervision.data.datamodules import AdataDataset
from self_supervision.models.lightning_modules.perturbations_autoencoder import (
    SCGENAutoEncoder,
)
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        choices=["Supervised", "Unsupervised", "scGEN"],
    )
    parser.add_argument(
...
def update_weights(pretrained_dir, model, model_type="AE"):
    """
    Update the weights of the model_dict based on possible matches in the pretrained dict
    Correct for some diverging layer-names
    :param pretrained_dir: Directory of the pretrained-weights
...
def old_setup_dataloader(args):
    """
    Setup the dataloader for the training script.
    """
    # Get Adata
    adata_train_val = os.path.join(
        args.adata_dir, "Srivatsan_2020_sciplex3_train_val_hvg.h5ad"
    )
    test_adata_dir = os.path.join(
        args.adata_dir, "Srivatsan_2020_sciplex3_test_hvg.h5ad"
    )
    adata = sc.read_h5ad(adata_train_val)
    ...
    # Couldn't train well with all perturbations -> make it much much easier
    ...
    adata_train = adata_train[adata_train.obs["perturbation"].isin(test_perturbations)]
...
def get_adata(args):
    train = sc.read_h5ad(
        os.path.join(args.adata_dir, "Srivatsan_2020_sciplex3_full_hvg.h5ad")
    )
    # Make similar to tutorial notebook
    train.obs["condition"] = pd.np.where(
        train.obs["perturbation"] == "control", "control", "stimulated"
    )
    train.obs["condition"]

    # We cannot leave perturbations as holdout test sets, because then either the train or the test set would not have control cells
    # Therefore simply hold out 1 cell line 1 celltype, very similar to scGEN
    train_new = train[
        ~(
            (train.obs["celltype"] == "lymphoblasts")
            & (train.obs["condition"] == "stimulated")
        )
    ].copy()

    # Balance the data
    train_new = balancer(train_new, "celltype")

    # Random split the train data into train and validation
    train_new.obs["split"] = np.random.choice(
        ["train", "val"], size=(train_new.shape[0]), p=[0.8, 0.2]
    )
    return train_new


def setup_dataloader(args, train_new):
    # Train and val dataset
    train_adata = train_new[train_new.obs["split"] == "train"]
    val_adata = train_new[train_new.obs["split"] == "val"]
    train_dataset = AdataDataset(
        genes=torch.tensor(train_adata.X.todense()),
        perturbations=train_adata.obs["perturbation"].values,
    )
    val_dataset = AdataDataset(
        genes=torch.tensor(val_adata.X.todense()),
        perturbations=val_adata.obs["perturbation"].values,
    )
...
if __name__ == "__main__":
    ...
    train_new = get_adata(args)

    # Setup dataloader
    train_dataloader, val_dataloader = setup_dataloader(args, train_new)
    ...
    elif args.model_type == "scGEN":
        model = SCGENAutoEncoder(
            gene_dim=gene_dim,
            units_encoder=[512, 512, 256, 256, 64],
            units_decoder=[256, 256, 512, 512],
            adata=train_new,
